...
function doStuff(e) {
	...
	if(!lengthStr){
		lengthStr = 44;
	}else if(lengthStr.slice(0,3).toLowerCase() == 'pin'){
		var isPin = true;
		var digits = lengthStr.match(/[0-9]/g);					//extract digits
		lengthStr = digits ? digits.join('') : 4
	}else if(lengthStr.match(/[^0-9]/)){
		mainMsg.textContent = "Password length must be a number";
		return
	}
  ...
  setTimeout(function(){														//the rest after a 0 ms delay
	if(pwdTable.style.display == 'block'){					//do passwords if the boxes are displayed, otherwise, just userName
		var pwdOut = [],			//compute the new password into an array
			newPwd = pwdStr1 ? pwdSynth(pwdStr1,serialStr1,isPin) : "";
	...
	//fill missing inputs and compute the rest of the passwords
		if(pwdNumber > 1){
			...
			newPwd = (serialStr2 == serialStr1) && (pwdStr2 == pwdStr1) ? pwdOut[0] : pwdSynth(pwdStr2,serialStr2,isPin);
			pwdOut.push(newPwd.slice(0,lengthStr))
		}
		if(pwdNumber > 2){
			if(!pwdStr3) pwdStr3 = pwdStr2;
			if(serialStr3 || serialStr2 == '-'){serialStr3 = serialStr3.trim()}else{serialStr3 = serialStr2};
			newPwd = (serialStr3 == serialStr2) && (pwdStr3 == pwdStr2) ? pwdOut[1] : pwdSynth(pwdStr3,serialStr3,isPin);
			pwdOut.push(newPwd.slice(0,lengthStr))
		}
		if(pwdNumber > 3){
			if(!pwdStr4) pwdStr4 = pwdStr3;
			if(serialStr4 || serialStr3 == '-'){serialStr4 = serialStr4.trim()}else{serialStr4 = serialStr3};
			newPwd = (serialStr4 == serialStr3) && (pwdStr4 == pwdStr3) ? pwdOut[2] : pwdSynth(pwdStr4,serialStr4,isPin);
			pwdOut.push(newPwd.slice(0,lengthStr))
		}
	  }
...
function pwdSynth(pwd, serial, isPin){
	...
	}else{							//base64, the replaces are to add generally accepted special characters
