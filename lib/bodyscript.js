window.onload = function() {
	var bgPage = chrome.extension.getBackgroundPage(),			//to reset cached password if the timer has elapsed
	...
	chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {	//find the id of the current tab and tell script to count password boxes
    	activeTab = tabs[0];
		chrome.tabs.sendMessage(activeTab.id, {"message": "start"});		//collect statistics from content script
